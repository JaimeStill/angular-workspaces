...
```bash
ng new smartid --createApplication=false --packageManager=yarn --skipTests=true -g -f
...
```bash
ng generate application demo --minimal=true --routing=true --skipTests=true --style=scss
```

```bash
ng generate application test --minimal=true --routing=true --skipTests=true --style=scss
```
